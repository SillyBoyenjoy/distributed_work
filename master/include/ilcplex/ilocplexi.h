...
#include <ilcplex/ilomodelasst.h>
...
public:
  ...
  IloNum getSlack (const IloConstraint con, IloInt soln=IncumbentId) const;
  ...
  enum NumParam {
    ...
    EpMrk           = CPX_PARAM_EPMRK,
    EpOpt           = CPX_PARAM_EPOPT,
    EpPer           = CPX_PARAM_EPPER,
    EpRHS           = CPX_PARAM_EPRHS,
    NetEpOpt        = CPX_PARAM_NETEPOPT,
    NetEpRHS        = CPX_PARAM_NETEPRHS,

    TiLim           = CPX_PARAM_TILIM,
    DetTiLim        = CPX_PARAM_DETTILIM,
    TuningTiLim     = CPX_PARAM_TUNINGTILIM,
    TuningDetTiLim  = CPX_PARAM_TUNINGDETTILIM,
    BtTol           = CPX_PARAM_BTTOL,
    CutLo           = CPX_PARAM_CUTLO,
    CutUp           = CPX_PARAM_CUTUP,
    EpGap           = CPX_PARAM_EPGAP,
    EpInt           = CPX_PARAM_EPINT,
    EpAGap          = CPX_PARAM_EPAGAP,
    EpRelax         = CPX_PARAM_EPRELAX,
    ObjDif          = CPX_PARAM_OBJDIF,
    ObjLLim         = CPX_PARAM_OBJLLIM,
    ObjULim         = CPX_PARAM_OBJULIM,
    PolishTime      = CPX_PARAM_POLISHTIME,
    PolishAfterEpAGap = CPX_PARAM_POLISHAFTEREPAGAP,
    PolishAfterEpGap  = CPX_PARAM_POLISHAFTEREPGAP,
    PolishAfterTime   = CPX_PARAM_POLISHAFTERTIME,
    PolishAfterDetTime = CPX_PARAM_POLISHAFTERDETTIME,
    ProbeTime       = CPX_PARAM_PROBETIME,
    ProbeDetTime    = CPX_PARAM_PROBEDETTIME,
    RelObjDif       = CPX_PARAM_RELOBJDIF,
    CutsFactor      = CPX_PARAM_CUTSFACTOR,
    TreLim          = CPX_PARAM_TRELIM,
    SolnPoolGap     = CPX_PARAM_SOLNPOOLGAP,
    SolnPoolAGap    = CPX_PARAM_SOLNPOOLAGAP,
    WorkMem         = CPX_PARAM_WORKMEM,

    BarEpComp       = CPX_PARAM_BAREPCOMP,
    BarQCPEpComp    = CPX_PARAM_BARQCPEPCOMP,
    BarGrowth       = CPX_PARAM_BARGROWTH,
    BarObjRng       = CPX_PARAM_BAROBJRNG,
    EpLin           = CPX_PARAM_EPLIN,
    RampupDettimeLim  = CPX_PARAM_RAMPUPDETTILIM,
    RampupWalltimeLim = CPX_PARAM_RAMPUPTILIM,

    BendersFeasCutTol = CPX_PARAM_BENDERSFEASCUTTOL,
    BendersOptCutTol  = CPX_PARAM_BENDERSOPTCUTTOL
  };
  ...
  enum BoolParam {
    ...
    ParamDisplay    = CPX_PARAM_PARAMDISPLAY,
    Record          = CPX_PARAM_RECORD,
    SiftingSimplex  = CPX_PARAM_SIFTSIM
  };
  ...
  enum IntParam {
    ...
    MultiObjDisplay = CPX_PARAM_MULTIOBJDISPLAY,
    ...
    BQPCuts         = CPX_PARAM_BQPCUTS,
    RLTCuts         = CPX_PARAM_RLTCUTS,
    ...
    ConflictAlg     = CPX_PARAM_CONFLICTALG,
    ...
    BendersStrategy = CPX_PARAM_BENDERSSTRATEGY,
    BendersWorkerAlg = CPX_PARAM_WORKERALG,
    DynamicRows     = CPX_PARAM_DYNAMICROWS,
    LocalImplied    = CPX_PARAM_LOCALIMPLBD,
    QToLin          = CPX_PARAM_QTOLININD,
    SolutionType    = CPX_PARAM_SOLUTIONTYPE,
    SubMIPScale     = CPX_PARAM_SUBMIPSCAIND,
    SubMIPStartAlg  = CPX_PARAM_SUBMIPSTARTALG,
    SubMIPSubAlg    = CPX_PARAM_SUBMIPSUBALG
  };
  ...
  enum LongParam {
    ...
    SubMIPNodeLimit = CPX_PARAM_SUBMIPNODELIMIT,
    WarningLimit    = CPX_PARAM_WARNLIM
  };
...
#define CPX_MIPSEARCH_DYNAMIC            2
  ...
  enum CalcQCPDuals {
   ...
   static const char * const BendersAnnotation;
  enum CplexStatus {
    Unknown         = 0,
  ...
  IloInt getMultiObjInfo(MultiObjIntInfo what, IloInt subprob) const;

  enum MultiObjInt64Info {
  ...
  IloInt64 getMultiObjInfo(MultiObjInt64Info what, IloInt subprob) const;


  enum MultiObjNumInfo {
  ...
  IloNum getMultiObjInfo(MultiObjNumInfo what, IloInt subprob) const;

  IloInt getMultiObjNsolves() const;
  ...
  public:
    Exception(int status, const char* str)
      : IloAlgorithm::Exception(str, IloTrue), _status(status)      {}
  ...
  public:
    ...
    enum Type {
      ...
      Tuning            = 21,
      LazyConstraint    = 22,
      _Number           = 23    // don't document; just name for last one
    };
...
#         include "iloCPXCALLBACKSOLUTIONSTRATEGYenum.h"
  ...
  void use(IloModelingAssistance::Callback *callback);
 ...
 */
  ...
  IloBool solve(const IloArray<ParameterSet> paramsets);
  ...
  static void alwaysCheckNAN(double val) {
     if ( val != val ) {
        throw IloCplex::Exception(CPXERR_NAN,
                "Numeric entry is not a double precision number (NAN)");
     }
  }
...
public:
  ...
  IloNum getMultiObjInfo(IloCplex::MultiObjNumInfo what, IloInt subprob) const;
  IloInt getMultiObjInfo(IloCplex::MultiObjIntInfo what, IloInt subprob) const;
  IloInt64 getMultiObjInfo(IloCplex::MultiObjInt64Info what, IloInt subprob) const;
  ...
  void setNumObjs  (IloInt num);
  void setMultiObj (IloInt n, const IloCarray<double>& obj, const IloCarray<Index>& col, IloNum offset,
                    double weight, int prio, double absTol, double relTol);
  IloBool isMultiObj();
  int getNumPrios();
  ...
  virtual IloBool solve(const IloArray<IloCplex::ParameterSet> params);
  virtual IloBool solve() { return solve(IloArray<IloCplex::ParameterSet>()); }
  ...
  static int makeParSet (CPXENVptr env, CPXPARAMSETptr *ps_p,
                         IloCplex::ParameterSetI const *parset);
...
inline void
IloCplex::use(IloModelingAssistance::Callback *callback) {
  IloAssert (getImpl(), "accessing IloCplex through 0 handle");
  return (getImpl()->use(callback));
}
