...
product CPLEX , library ilocplex.lib, version 12.9.0.0, date 03/08/2019, \
Copyright IBM Corporation 2000, 2019. All Rights Reserved. \
US Government Users Restricted Rights - Use, duplication or \
disclosure restricted by GSA ADP Schedule Contract with IBM Corp."
#else
#define ILOCPLEX_VERSIONSTRING "@(#)Licensed Materials - Property of IBM - 5725-A06 5725-A29 5724-Y48 5724-Y49 5724-Y54 5724-Y55 5655-Y21: \
product CPLEX , library libilocplex.a, version 12.9.0.0, date 03/08/2019, \
Copyright IBM Corporation 2000, 2019. All Rights Reserved. \
US Government Users Restricted Rights - Use, duplication or \
disclosure restricted by GSA ADP Schedule Contract with IBM Corp."
#endif
...
inline IloBool
IloCplex::solve(const IloArray<IloCplex::ParameterSet> paramsets) {
  IloAssert(getImpl() != 0, "accessing IloCplex through 0 handle");
  return getImpl()->solve(paramsets);
}
...
inline IloNum
IloCplex::getSlack (const IloConstraint con, IloInt soln) const {
  IloAssert (getImpl() != 0, "accessing IloCplex through 0 handle");
  IloAssert (con.getImpl() != 0, "accessing IloRange through 0 handle");
  if ( con.getImpl()->getTypeIndex() != IloRangeI::GetTypeIndex() )
     throw IloWrongUsage ("IloCplex::getSlack must take an IloRange as argument");
  return (getImpl()->getSlack((IloRangeI *)con.getImpl(), soln));
}
...
inline IloNum
IloCplex::getMultiObjInfo(IloCplex::MultiObjNumInfo what, IloInt subprob) const {
  IloAssert (getImpl() != 0, "accessing IloCplex through 0 handle");
  return (getImpl()->getMultiObjInfo(what, subprob));
}

inline IloInt
IloCplex::getMultiObjInfo(IloCplex::MultiObjIntInfo what, IloInt subprob) const {
  IloAssert (getImpl() != 0, "accessing IloCplex through 0 handle");
  return (getImpl()->getMultiObjInfo(what, subprob));
}

inline IloInt64
IloCplex::getMultiObjInfo(IloCplex::MultiObjInt64Info what, IloInt subprob) const {
  IloAssert (getImpl() != 0, "accessing IloCplex through 0 handle");
  return (getImpl()->getMultiObjInfo(what, subprob));
}

inline IloInt
IloCplex::getMultiObjNsolves() const {
  IloAssert (getImpl() != 0, "accessing IloCplex through 0 handle");
  return (getImpl()->getMultiObjNsolves());
}
