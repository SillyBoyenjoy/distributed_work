    ...
    while ( 1 ) {
        ...
        if ( (GOLBALGAP < 3.00) && (cut == 1) ){
            ...
            if ( (j = BestDuals[0].idx) != i ){ 
                ...
                status = CPXXwriteprob( env[j], lp[j], "./data/BestDual.gz", NULL );
                if ( status != 0 ){
                    fprintf (stderr, "CPXXsolwrite: %d\n", status);
                    abort ();
                }
            ...
            /* Primal and Dual is the same worker */
                ...
                printf("Worker %d is writing the best dual bound...\n", i);
                status = CPXXwriteprob(env[i], lp[i], "./data/BestDual.gz", NULL);
                if ( status != 0 ){
                    fprintf (stderr, "CPXXsolwrite: %d\n", status);
                    abort ();
                }
            ...
            for ( i = 0; i < jobs; i++ ){
                ...
                if ( i == 0 ){                /* A dual worker every 5 */
                    status = CPXXreadcopyprob(env[i], lp[i], "./sol/BestDual.gz", NULL);
                    if ( status != 0 ){
