...
#define MAXDIFF				(remotestats[jobs-1].primal - remotestats[0].primal)/remotestats[jobs-1].primal
...
applySettings (CPXENVptr env, int setting)
{
    ...
    if ( setting >= 3 ){
        ...
        printf("Set worker %d param: %s.\n", 
               remotestats[setting].idx, remotestats[setting].paramname);
    }
    else{
...
static int PriCompare (const void *x, const void *y){
    double primal_x = ((RmtStatPtr)x)->primal;
    double primal_y = ((RmtStatPtr)y)->primal;
             ...
             void const *data, void *handle)
{
	...
	case INFO_NEWPRIMAL:
        ...
        qsort(remotestats, jobs, sizeof(struct remotestat), PriCompare);
		break;
	default:
		fflush (stderr);
	}
    ...
    /**/
    for (i=0; i < jobs; i++)
        printf("worker %d: primal %e, gap %3.2f%%.\n",
              remotestats[i].idx, remotestats[i].primal, remotestats[i].gap);
...
main(int argc, char **argv){
    ...
    char cmdbuf[MAX_CMD_LEN] = NULL;
    CPXASYNCptr *handle;
    CPXENVGROUPptr group;
    int i, j;
    int warm_start = 0, iteration = 0, max_jobs = 0, qflag = 0;
    int active, *conn;
    char const **machine;
   ...
   if ( machine == NULL ) {
    ...
    for (i = 1; i < argc; ++i) {
        ...
        else if ( strncmp (argv[i], "-warmstart=./data/", 18) == 0 ){
            warm_start = 1;
            startfile = argv[i] + 11;
    ...
    while (active > 0) {
        ...
        if ( (primal.valid && dual.valid && GOLBALGAP <= 100*gap) || (qflag == 1) ){
		...
		if ( ((cnt++) > 120000) && ((MAXDIFF) > 0.2) && (iteration == 1) ){
            ...
            if ( (status = CPXXsolwrite( env[i], lp[i], "./data/incumbent.gz" )) != 0 ){
            ...
            if ( (status = CPXXreadcopymipstarts(env[j], lp[j], "./data/incumbent.gz")) != 0){
