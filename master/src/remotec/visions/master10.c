...
main(int argc, char **argv){
    ...
    int i, j;
    int warm_start = 0, iteration = 0, max_jobs = 0, polish = 0;
    int active, *conn;
    char const **machine;
    ...
    if ( warm_start == 1 ){
        ...
        if ( polish == 1 ){
            ...
            if ( (CPXXsetdblparam(env[j], CPXPARAM_MIP_PolishAfter_Time, 0)) != 0 )
    ...
    while (active > 0) {
		...
		if ( ((cnt++) > 30000) && (remotestats[jobs-1].gap > 5) && 
             (remotestats[0].gap < 3.00) && (iteration == 1) ){
