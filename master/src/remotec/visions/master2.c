...
static struct paramvalue defaultbound[] = {
	DOUBLEPARAM (CPXPARAM_MIP_Tolerances_MIPGap, 0.01), /* Set mipgap as 1% */
	ENDPARAM
};
             ...
             void const *data, void *handle)
{
	...
	case INFO_DETTIME:
		...
		assert (length == 1);
		d = *(double const *)data;
		printf ("[%d] Current dettime: %f (%e, %e), gap: %3.2f%%\n",
				rs->idx, d, rs->dual, rs->primal, LOCALGAP);
		fflush (stderr);
		rs->dettime = d;
...
main(int argc, char **argv)
{
	...
	* machine.
	*/
	status = CPXXsetintparam_multicast (group, CPXPARAM_Threads, 2);
	if ( status != 0 ) {
	...
	for (i = 0; i < jobs; ++i) {
		...
		if ( (status = CPXXreadcopyprob_async (env[i], lp[i], modelfile, NULL, &handle[i])) != 0 ) {
			fprintf (stderr, "CPXXreadcopyprob_async: %d\n", status);
			abort ();
		}
	}
	objsen = CPXXgetobjsen (env[0], lp[0]);
	...
	active = jobs;
	frequency = 50000; /* Print current bounds every two seconds. */
	while (active > 0) {
		int running = 0;
