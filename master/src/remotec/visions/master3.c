...
printtime (int fg){
    ...
    if ( fg == 1 ){
	    ...
	    fprintf(log, "The clock time is %f sec.\n", time);
    }
	fclose(log);
	return time;
}
             ...
             void const *data, void *handle)
{
	...
	case INFO_NEWDUAL:
        ...
        fprintf (log, "[%d] New dual bound: %e\n", rs->idx, d);
		fflush (stderr);
		rs->dual = d;
		if ( !dual.valid ||
			...
			(objsen == CPX_MAX && d < dual.bound) )
		{
            ...
            fprintf (log, "[%d] Bound updated: (%e, %e), gap: %3.2f%%.\n", rs->idx, 
                    dual.bound, primal.bound, GOLBALGAP);
		}
		printtime(1);
		break;
	case INFO_NEWPRIMAL:
...
#   error "No transport type selected"
    ...
    for (i = 0; i < jobs; ++i) {
        ...
        switch (output) {
            ...
            break;
            case OUTPUT_LOG:{
                if ( (status = CPXXsetintparam (env[i], CPXPARAM_ScreenOutput, 
                                                CPX_ON)) != 0 ) {
                    fprintf (stderr, "CPXXgetchannels: %d\n", status);
                    abort ();
                }
    ...
    */
    ...
    printf ("Setting the threads.\n");
    fprintf (log, "Setting the threads.\n");
    if ( status != 0 ) {
