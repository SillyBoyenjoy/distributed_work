...
commandinput (void){
    ...
    close_keyboard();
    pthread_exit("Optimize finished.\n");
}
...
main(int argc, char **argv){
    ...
    while ( 1 ) {
        ...
        /* Branch and cut if golbalgap < 5.00%. */
        if ( (GOLBALGAP < 3.00) && (cut == 1) ){
            ...
            if ( (status = CPXXsolwrite( env[i], lp[i], "./data/BestPrimal.gz" )) != 0 ){
                fprintf (stderr, "CPXXsolwrite: %d\n", status);
                abort ();
            }
            printf ("Worker writing completed.\n");

            /* If Primal and Dual in different worker */
            if ( (j = BestDuals[0].idx) != i ){ 
                printf("Worker %d is writing the best dual bound...\n", j);
                status = CPXXsolwrite( env[j], lp[j], "./data/BestDual.gz" );
                if ( status != 0 ){
                    fprintf (stderr, "CPXXsolwrite: %d\n", status);
                    abort ();
                }
            }
            /* Primal and Dual is the same worker */
            else{                               
                printf("Worker %d is writing the best dual bound...\n", i);
                status = CPXXsolwrite(env[i], lp[i], "./data/BestDual.gz");
                if ( status != 0 ){
                    fprintf (stderr, "CPXXsolwrite: %d\n", status);
                    abort ();
                }
            ...
            for ( i = 0; i < jobs; i++ ){
                ...
                if ( i == 0 ){                /* A dual worker every 5 */
                    status = CPXXreadcopymipstarts(env[i], lp[i], "./sol/BestDual.gz");
                    if ( status != 0 ){
                    ...
                    status = CPXXreadcopymipstarts(env[i], lp[i], "./sol/BestPrimal.gz");
                    if ( status != 0 ){
                        fprintf (stderr, "CPXXreadcopymipstarts: %d\n", status);
                        abort ();
                    }
